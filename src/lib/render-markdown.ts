...
/* 一键复制按钮与语言标签：按钮行为由全局委托监听（code-copy-listener）处理，
   样式在 globals.css（.code-copy / .code-lang），DOMPurify 默认白名单允许
   button 和 span 存活 */
/* 不要加 aria-label="复制代码"：按钮的可见文字是 "copy"，无障碍名里不含可见文字
   会触发 WCAG 2.5.3（label-content-name-mismatch），语音控制说"复制代码"点不动它。
   可见文字本身就是无障碍名，中文说明放 title 当 tooltip */
const COPY_BTN = '<button type="button" class="code-copy" title="复制代码">copy</button>';
...
const md = new MarkdownIt({
  ...
  highlight: (str: string, lang: string) => {
    ...
    // 不标注 —— 否则纯 ``` 代码块左上角会莫名其妙标个 "ini"
    const userLang = hljs.getLanguage(lang) ? lang : "";
    const displayLang = userLang || "";
    const langName = userLang || (hljs.getLanguage('ini') ? 'ini' : '');
    if (langName) {
      try {
        return `<pre class="hljs">${langLabel(displayLang)}${COPY_BTN}<code>${hljs.highlight(str, { language: langName, ignoreIllegals: true }).value}</code></pre>`;
      } catch {}
    }
    return `<pre class="hljs">${langLabel(displayLang)}${COPY_BTN}<code>${escapeHtml(str)}</code></pre>`;
  },
});
