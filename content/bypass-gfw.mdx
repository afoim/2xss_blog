...
draft: true
coverImage: https://raw-posts.2x.nz/img/bypass-gfw-Snipaste_2024-10-21_19-36-34.webp
---
...
- 首先尝试访问 `www.baidu.com` 这是一个没有被GFW封锁的域名
  ...
  1. 我们先ping一下![](https://raw-posts.2x.nz/img/bypass-gfw-2024-10-21-20-16-48-image.webp)
  
  2. 得到ip： `2408:873d:22:18ac:0:ff:b021:1393` 
  
  3. 通过Hosts强制绑定![](https://raw-posts.2x.nz/img/bypass-gfw-2024-10-21-20-18-10-image.webp)
  
  4. 通过WireShark进行抓包，可以看到，客户端发送的 `Client Hello` 可以清晰地看到 `Server Name` 字段，并且也能正常收到 `Server Hello` 然后双方便开始通信![](https://raw-posts.2x.nz/img/bypass-gfw-2024-10-21-20-24-03-image.webp)
  
  5. 查看浏览器，网站正常访问![](https://raw-posts.2x.nz/img/bypass-gfw-2024-10-21-20-35-29-image.webp)

- 让我们试试访问 `discord.com`
  
  1. 我们先ping一下，可以发现，域名和解析到的IP均不通![](https://raw-posts.2x.nz/img/bypass-gfw-2024-10-21-20-27-57-image.webp)
  
  2. 此时我们尝试使用 `itdog.cn` 进行v4 ping，并且依次对解析出的域名进行ping![](https://raw-posts.2x.nz/img/bypass-gfw-2024-10-21-20-28-51-image.webp)
  
  3. 可见，第一个IP通![](https://raw-posts.2x.nz/img/bypass-gfw-2024-10-21-20-29-40-image.webp)
  
  4. 强制绑定Hosts，尝试抓包![](https://raw-posts.2x.nz/img/bypass-gfw-2024-10-21-20-35-58-image.webp)![](https://raw-posts.2x.nz/img/bypass-gfw-2024-10-21-20-31-49-image.webp)
  
  5. 可见，在通过强制Hosts绑定后，在客户端发送 `Client Hello` 后被GFW检测到`Server Name` 字段，然后GFW向客户端发送一个 `RST` 报文，即要求重置客户端连接。在客户端侧，则会收到 `ERR_CONNECTION_RESET` 即：连接已重置。用户无法访问网页。![](https://raw-posts.2x.nz/img/bypass-gfw-2024-10-21-20-33-23-image.webp)

#### 继续，尝试发送空 `Server Name` 报文

![](https://raw-posts.2x.nz/img/bypass-gfw-2024-10-21-20-41-37-image.webp)

![](https://raw-posts.2x.nz/img/bypass-gfw-2024-10-21-20-41-54-image.webp)
...
- 直接打开这个软件，看到这个画面即可![](https://raw-posts.2x.nz/img/bypass-gfw-c2eed28c-6e5d-43a3-a016-8f1a38a53cbd.webp)

- 它的原理是自动设置系统代理，如果你使用了一些别的代理软件，会被覆盖![](https://raw-posts.2x.nz/img/bypass-gfw-d0d8fac1-a2e5-4db2-8e25-ca5e04eb9951.webp)
