...
draft: false
coverImage: https://raw-posts.2x.nz/img/umami-migration-umami-migration.webp
---
## 正式开始
首先，我们想要迁移，首先肯定是要拿到原来的Umami Cloud存放的数据

在 [Umami Cloud Data | Settings](https://cloud.umami.is/settings/data) 中，我们可以选择，导出数据（Export）
![](https://raw-posts.2x.nz/img/umami-migration-umami-migration-1.webp)

当数据准备好后（几分钟左右），你的邮箱会收到一封Umami Cloud发来的邮件
![](https://raw-posts.2x.nz/img/umami-migration-umami-migration-2.webp)

下载下来的文件是一个以UUID命名的ZIP压缩包，将其解压可以得到3个CSV文件
![](https://raw-posts.2x.nz/img/umami-migration-umami-migration-3.webp)
...
然后连上数据库，你将可以看到这些表
![](https://raw-posts.2x.nz/img/umami-migration-umami-migration-4.webp)

接下来我们查看 `website` 这张表，将 `website_event.csv` 打开，查看旧的 `website_id` 将新的ID改为旧的，这样才能数据匹配
![](https://raw-posts.2x.nz/img/umami-migration-umami-migration-5.webp)

![](https://raw-posts.2x.nz/img/umami-migration-umami-migration-6.webp)
...
![](https://raw-posts.2x.nz/img/umami-migration-umami-migration-7.webp)

编辑CSV文件，你需要将顺序配对，并且删除表中没有的列，如： `browser` `os` ...
![](https://raw-posts.2x.nz/img/umami-migration-umami-migration-8.webp)

确保数据库中的表头顺序和CSV文件中的表头顺序一一对应，顺序一致，不多不少后，**Ctrl + S 保存** CSV文件，我们开始导入并在选项中勾选 **标题**

![](https://raw-posts.2x.nz/img/umami-migration-umami-migration-9.webp)

![](https://raw-posts.2x.nz/img/umami-migration-umami-migration-10.webp)

![](https://raw-posts.2x.nz/img/umami-migration-umami-migration-11.webp)
导入成功后如图
![](https://raw-posts.2x.nz/img/umami-migration-umami-migration-12.webp)

![](https://raw-posts.2x.nz/img/umami-migration-umami-migration-13.webp)

再编辑 `session` 这张表，手法同上，导入成功后如图
![](https://raw-posts.2x.nz/img/umami-migration-umami-migration-14.webp)

接下来打开本地Umami的WebUI，查看数据是否有异常
![](https://raw-posts.2x.nz/img/umami-migration-umami-migration-15.webp)
